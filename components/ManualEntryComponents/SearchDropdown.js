...
export default function SearchDropDown({filtered, setSelectedList, setSearchInput, setFilteredList, setSearchStatus}) {
  ...
  const filteredDisplay = filtered.slice(0,5).map((x) =>
    <Button minW="200" key={x.id} onPress={() => handleSelect(x)}>{x.name}</Button>
  )
